...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        Returns:
            batch_dict:
                ...
                rcnn_cls_labels: (B, M)
        """
        # [4, 128, 7], [4, 128, 8], [4, 128], [4, 128], [4, 128]
        batch_rois, batch_gt_of_rois, batch_roi_ious, batch_roi_scores, batch_roi_labels = self.sample_rois_for_rcnn(
            batch_dict=batch_dict
        )
        # regression valid mask, 每个 batch 前面是 fg，后面是 bg
        reg_valid_mask = (batch_roi_ious > self.roi_sampler_cfg.REG_FG_THRESH).long()

        # classification label
        if self.roi_sampler_cfg.CLS_SCORE_TYPE == 'cls':    # False
            ...
            # 对中间模糊区域，使用线性插值指示其置信度（应该是置信度吧），更准确地反映出这个RoI是前景的可能性
    ...
    def subsample_rois(self, max_overlaps):
        ...
        # fg_rois_per_image = 64, fg_thresh = 0.55
        fg_rois_per_image = int(np.round(self.roi_sampler_cfg.FG_RATIO * self.roi_sampler_cfg.ROI_PER_IMAGE))
        fg_thresh = min(self.roi_sampler_cfg.REG_FG_THRESH, self.roi_sampler_cfg.CLS_FG_THRESH)
        # 之前挑选的是NMS处理后的锚框，因此置信度高，但不一定是物体
        # easy: 容易被模型识别为背景的区域(与 gt IoU 较低），后期出现会比较少
        # hard: 与前景有某种程度的相似性，但实际上是背景，可以作为负样本，使模型更健壮
        # thresh: easy_bg = 0:0.1, hard_bg = 0.1:0.55, fg_inds = fg_thresh:
        ...
        # 注意这里是用 gt 分割了前后景
        fg_num_rois = fg_inds.numel()   # 前景 RoI 数量
        bg_num_rois = hard_bg_inds.numel() + easy_bg_inds.numel()   # 背景 RoI 数量

        if fg_num_rois > 0 and bg_num_rois > 0:     # True
            # sampling fg
            fg_rois_per_this_image = min(fg_rois_per_image, fg_num_rois)    # 64
            # 生成1到78的整数数组（不重复），然后随机排列
            rand_num = torch.from_numpy(np.random.permutation(fg_num_rois)).type_as(max_overlaps).long()
            fg_inds = fg_inds[rand_num[:fg_rois_per_this_image]]    # 随机选择前 64 项

            # sampling bg, bg_rois_per_this_image = 64
    ...
    def get_max_iou_with_same_class(rois, roi_labels, gt_boxes, gt_labels):
        ...
        for k in range(gt_labels.min().item(), gt_labels.max().item() + 1):
            ...
            gt_mask = (gt_labels == k)
            if roi_mask.sum() > 0 and gt_mask.sum() > 0:    # True
